...
import { ArrowLeft, ArrowRight, RepeatIcon, Share2, Shuffle } from "lucide-react";
...
export default function LearnClient({
    ...
    const handleShare = async () => {
        ...
        const invitor = username || "Ai đó";
        const lang = collection.language;
        const shareUrl = `${baseUrl}?title=${title}&invitor=${invitor}&lang=${lang}`;

        try {
            await navigator.clipboard.writeText(shareUrl);
            setShared(true);
            toast.success("Copy link thành công! Hãy chia sẻ với bạn bè!", {
                position: "top-center",
            });
        } catch (err) {
            console.error("", err);
